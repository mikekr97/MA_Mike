...
DIR = 'runs/experiment_4_simulation_linear/run'
if (!dir.exists(DIR)) {
  dir.create(DIR, recursive = TRUE)
}
# Copy this file to the directory DIR
file.copy('MA_Mike/experiment_4_simulation_linear.R', file.path(DIR, 'experiment_4_simulation_linear.R'), overwrite=TRUE)
...
if (F42 == 1){
  ...
  f = function(x) 0.5*atan(5*(x-0.32))
  FUN_NAME = 'DPGatan'
} else if (F42 == 5){
...
true_betas <- c(0.2, -0.3, 0.35, -0.3, -0.7)
...
if (FALSE){
  ...
  file_path <- file.path("runs/experiment_4_simulation_linear/run/", basename(file_name))
...
for (i in 1:length(xs)){
  ...
  #cs_24[i] = param_model(X)[1,4,1]$numpy() #1=CS Term X1->X2
}
par(mfrow=c(1,4))
...
# plot(xs, cs_24, type='l', col='red', lwd=2, xlab='x1', ylab='f(x1)', main='b12(x1) complex')
...
if (TRUE){
  ...
  h4_DGP_LS = (-0.3) * X_1 + f(X_2) + (-0.7) * X_3
  h4_DGP_I = 5*X_4
