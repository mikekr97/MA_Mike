...
\begin{enumerate}
    ...
    \item \textsb{Ignorability/Unconfoundedness:} $(Y(1), Y(0)) \perp T \mid X$ \\
    Treatment assignment is independent of potential outcomes given observed covariates
    \item \textsb{Overlap/Positivity:} $0 < \text{P}(T = 1 \mid X = x) < 1 \quad \text{for all } x$ \\
...
\begin{itemize}
    \item \textsb{T-learners:} Two models, separately estimated on treated and control groups
      \begin{itemize}
        \item Logistic regression
        \item Random forest (with hyperparameter tuning)
      \end{itemize}
    \item \textsb{S-learners:} One model, with treatment as a feature
      \begin{itemize}
...
\textsb{Research question:} Do we reach similar conclusion as \citet{chen2025} when estimating ITEs with T-learners (logistic regression, tuned random forest) and S-learner (TRAM-DAGs) on IST dataset.
...
\textsb{Research question:} What factors contribute to the failure of ITE estimation in causal ML models? %, and under which conditions can reliable estimates be obtained?
