...
\textbf{Setup:}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item Sample from conditional distributions for causal queries with structural equations $X_i = h_i^{-1}(Z_i \mid \operatorname{pa}(X_i))$
    \end{itemize}
  \end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{No interference:} one person's treatment doesn't affect another's outcome
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item \textsb{S-learner:} 
    ...
    \end{enumerate}
    \begin{itemize}
      \item TRAM-DAGs (SCM, flexible, interactions, generative)
    \end{itemize}
\end{itemize}
