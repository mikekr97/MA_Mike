...
\begin{columns}
  ...
  \begin{column}{0.4\textwidth}
    \centering
    \includegraphics[width=\linewidth]{img/Experiment1/dag_exp1.png}
  \end{column}
...
h(X_{3,k} \mid X_1, X_2) = \vartheta_k + \textcolor{red}{\beta_{13}} X_1 + \textcolor{blue}{f(X_2)} \\[0.8em] 

\boxed{
\textcolor{blue}{f(X_2) = 0.5 \cdot \exp(X_2)}
}
\end{array}
\]
...
\textbf{We want:}
  \begin{itemize}
    \item With TRAM-DAGs, estimate $Z_i = h_i(X_i \mid \operatorname{pa}(X_i))$ of each variable $i$
    \item Sample from fitted model to make causal queries
  \end{itemize}
...
\begin{minipage}{\linewidth}
  \small Sampled \textbf{Observational} distribution: \\[0.2em]
  \includegraphics[width=0.8\linewidth]{img/experiment1/exp1_observational_distribution.png}
\end{minipage}



\begin{minipage}{\linewidth}
  \small Sampled \textbf{Interventional} distribution; do($X_2 = 1$): \\[0.2em]
  \includegraphics[width=0.8\linewidth]{img/experiment1/exp1_interventional_distribution.png}
\end{minipage}
...
% \vspace{0.5em}
\textbf{Definition:}  \textit{Individual treatment effect} \citep{rubin2005}
\[
Y_i(1) - Y_i(0)
\]
...
\small
\textbf{Goal:} Define the \textit{individualized treatment effect (ITE/CATE)} estimand, which we aim to estimate from observed data \citep{hoogland2021}.
...
\textbf{Results:} with T-learner \textsb{tuned random forest} using the \texttt{comets} package \citep{comets}:
...
\begin{itemize}
    ...
    \item Ground truth is unknown -- difficult to determine if no true heterogeneity present or models fail to capture it
\end{itemize}
...
\begin{frame}{Simulation Case 1: \textcolor{red}{Fully Observed}}
...
\begin{itemize}\setlength\itemsep{0.4em}
  ...
  \item $\mathbf{X_{TX}} = (X_1, X_2)^\top$ \textcolor{red}{interacting variables}
\end{itemize}
...
Results with T-learner \textsb{logistic regression} (glm):
\vfill

\begin{center}
\begin{tikzpicture}
    \node[anchor=south west, inner sep=0] (image) at (0,0) {\includegraphics[width=\textwidth]{img/fully_observed_glm_tlearner.png}};
    \begin{scope}[x={(image.south east)}, y={(image.north west)}]
        % Red rectangle (adjust coordinates to match area you want to box)
        \draw[red, thick, rounded corners] (0.27,0.03) rectangle (0.5,1.0);
    \end{scope}
\end{tikzpicture}
\end{center}
...
\textbf{Interpretation:} Accurate ITE estimation!
...
Results with T-learner \textsb{tuned random forest} (\texttt{comets} package):
...
\begin{center}
\begin{tikzpicture}
    \node[anchor=south west, inner sep=0] (image) at (0,0) {\includegraphics[width=\textwidth]{img/observed_tuned_rf.png}};
    \begin{scope}[x={(image.south east)}, y={(image.north west)}]
        % Red rectangle (adjust coordinates to match area you want to box)
        \draw[red, thick, rounded corners] (0.27,0.03) rectangle (0.5,1.0);
    \end{scope}
...
\textbf{Interpretation:} Unbiased ITE estimation!
...
Results with \textsb{(untuned) T-learner random forest} using the \texttt{randomForest} package \citep{breiman2001}:
...
\begin{center}
\begin{tikzpicture}
    \node[anchor=south west, inner sep=0] (image) at (0,0) {\includegraphics[width=\textwidth]{img/fully_observed_rf_tlearner.png}};
    \begin{scope}[x={(image.south east)}, y={(image.north west)}]
        % Red rectangle (adjust coordinates to match area you want to box)
        \draw[red, thick, rounded corners] (0.0,0.55) rectangle (0.27,1.0);
    \end{scope}
\end{tikzpicture}
\end{center}
...
\textbf{Interpretation:} Overfitted, poorly calibrated, leads to worse ITE estimates
...
\begin{frame}{Simulation Case 2: \textcolor{red}{Unobserved Interaction}}
...
\begin{center}
\begin{tikzpicture}
    \node[anchor=south west, inner sep=0] (image) at (0,0) {\includegraphics[width=\textwidth]{img/unobserved_interaction_glm_tlearner.png}};
    \begin{scope}[x={(image.south east)}, y={(image.north west)}]
        % Red rectangle (adjust coordinates to match area you want to box)
        \draw[red, thick, rounded corners] (0.27,0.03) rectangle (0.5,1.0);
    \end{scope}
...
\textbf{Interpretation:} 1) Model misses positive ITEs, 2) ITE-ATE plot misleading -- suggests good calibration, but doesn't detect patients that benefit!
...
\begin{center}
\begin{tikzpicture}
    \node[anchor=south west, inner sep=0] (image) at (0,0) {\includegraphics[width=\textwidth]{img/unobserved_tuned_rf.png}};
    \begin{scope}[x={(image.south east)}, y={(image.north west)}]
        % Red rectangle (adjust coordinates to match area you want to box)
        \draw[red, thick, rounded corners] (0.27,0.03) rectangle (0.5,1.0);
    \end{scope}
...
\textbf{Interpretation:} Similar problem as with logistic model!
...
\begin{frame}{Simulation Case 3: \textcolor{red}{Fully Observed, Small Effects}}
...
\begin{center}
\begin{tikzpicture}
    \node[anchor=south west, inner sep=0] (image) at (0,0) {\includegraphics[width=\textwidth]{img/small_interaction_glm_tlearner.png}};
    \begin{scope}[x={(image.south east)}, y={(image.north west)}]
        % Red rectangle (adjust coordinates to match area you want to box)
        \draw[red, thick, rounded corners] (0.27,0.03) rectangle (0.5,1.0);
    \end{scope}
...
\textbf{Interpretation:} 1) Predicts too large heterogeneity (model noise?),\\ 2) ITE-ATE plot correctly suggests no significant heterogeneity!
...
\begin{center}
\begin{tikzpicture}
    \node[anchor=south west, inner sep=0] (image) at (0,0) {\includegraphics[width=\textwidth]{img/small_interaction_tuned_rf_tlearner.png}};
    \begin{scope}[x={(image.south east)}, y={(image.north west)}]
        % Red rectangle (adjust coordinates to match area you want to box)
        \draw[red, thick, rounded corners] (0.27,0.03) rectangle (0.5,1.0);
    \end{scope}
...
\begin{itemize}
  ...
  \item Ignorability (unconfoundedness) assumption alone may not guarantee unbiased ITEs if important \textsb{effect modifiers are unobserved}
  \item In practice, only the \textsb{ITE-ATE plot} is available -- it checks ITE calibration in predicted subgroups, but can miss true effect heterogeneity
