...
\chapter{Experiment 4: ITE estimation with TRAM-DAGs (simulation)} \label{ch:exp4}
...
$\Sexpr{sprintf('%.3f', rct_scenario1$dev_ATE_observed_Y_mean_diff)}$, with a 95\% Wald confidence interval of 
...
$\Sexpr{sprintf('%.3f', rct_scenario2$dev_ATE_observed_Y_mean_diff)}$, with a 95\% Wald confidence interval of 
$\Sexpr{sprintf('%.3f', rct_scenario2$dev_ATE_observed_Y_mean_diff_CI[1])}$ to 
$\Sexpr{sprintf('%.3f', rct_scenario2$dev_ATE_observed_Y_mean_diff_CI[2])}$. 
