...
MA =  matrix(c(
  0,   0,  0, 0, 0, 0, 'ci',    ## X1 impacts X4 (Tr) and X7 (Y) directly
  0,   0,  0, 0, 0, 0, 'ci',    ## X2 impacts X4 (Tr) and X7 (Y) directly
...
###############################################################################
# Check ITE DGP with expected value sampling instead of median.
###############################################################################


##### DGP ########
dgp_simulation_expected <- function(n_obs=20000, 
                           SEED=123, 
                           rho = 0.1, 
  ...
  # par(mfrow=c(1,2))
  # hist(X7_tx - X7_ct, xlim = c(-0.75, -0.1), breaks=50)
  # hist(ITE_median, xlim = c(-0.8, -0.1), breaks=50)
  # 
  # plot(X7_tx - X7_ct, ITE_median)
  # 
  # qqplot(X7_tx - X7_ct, ITE_median)
  # abline(0,1)
  ...
  # X1, X2, X3, X4 (Tr), X5, X6, X7 (Y)
  A =  matrix(c(
    0,   0,  0,  0,  0,  0,  1,    ## X1 impacts X4 (Tr) and X7 (Y) directly
    0,   0,  0,  0,  0,  0,  1,    ## X2 impacts X4 (Tr) and X7 (Y) directly
...
train <- dgp_simulation_expected(n_obs = n_obs, SEED = 123, rho = 0.1, doX = c(NA, NA, NA, NA, NA, NA, NA),
...
hist(train$simulated_full_data$ITE_expected, main = "ITE Expected", xlab = "ITE Expected", breaks = 50)
abline(v=ATE_mean)
...
mean(train$simulated_full_data$ITE_median)
...
## attention: changed to confounded
dgp_simulation <- function(n_obs=20000, 
  ...
  if (is.na(doX[4])) {
    ...
    beta_0 <- 0.5
    beta_X_Tr <- c(-0.8, 1.5)  # main effects of X1 and X2
  ...
  if (is.na(doX[7])){
    ...
    data_X_ct <- as.matrix(data.frame(X1 = data[,1], X2 = data[,2], X3 = data[,3], X5 = X5_ct, X6 = X6_ct))
    logit_X7_ct <- beta0 + data_X_ct %*% beta_X
    X7_ct <- 0.3*(x_7_dash - logit_X7_ct)
    
    # under Tr = 1
    data_X_tx <- as.matrix(data.frame(X1 = data[,1], X2 = data[,2], X3 = data[,3], X5 = X5_tx, X6 = X6_tx))
    logit_X7_tx <- beta0 + beta_t + data_X_tx %*% beta_X + (data_X_tx[,c("X2", "X3")] %*% beta_TX)
    X7_tx <- 0.3*(x_7_dash - logit_X7_tx)
...
# confounded treatement:
plot(table(train$simulated_full_data$Tr))
...
### attention, changed to confounded
dgp_simulation_no_mediators <- function(n_obs=20000, 
                           SEED=123, 
                           rho = 0.1, 
                           doX=c(NA, NA, NA, NA, NA),
                           main_effect = TRUE,
                           interaction_effect = TRUE,
  ...
  #### Y (X7): continuous outcome dependent on Tr, X1, X2, X3, (incl. Tr interactions between X2, X3)
  
  data_X <- as.matrix(data.frame(X1 = data[,1], X2 = data[,2], X3 = data[,3]))
  ...
  if (is.na(doX[7])){
    ...
    beta_X <- c(-0.5, 0.5, 0.2)  # direct effects of X1, X2, X3, X5, X6
    ...
    X7 = 0.3*(x_7_dash - logit_X7)
    
    # under Tr = 0
    data_X_ct <- as.matrix(data.frame(X1 = data[,1], X2 = data[,2], X3 = data[,3]))
    logit_X7_ct <- beta0 + data_X_ct %*% beta_X
    X7_ct <- 0.3*(x_7_dash - logit_X7_ct)
    
    # under Tr = 1
    data_X_tx <- as.matrix(data.frame(X1 = data[,1], X2 = data[,2], X3 = data[,3]))
    logit_X7_tx <- beta0 + beta_t + data_X_tx %*% beta_X + (data_X_tx[,c("X2", "X3")] %*% beta_TX)
    X7_tx <- 0.3*(x_7_dash - logit_X7_tx)
  ...
  # mean(h_y_inverse(X7_samples[1,] - logit_X7_ct[1]))
  # 
  mean(0.3*(X7_samples[1, ] - logit_X7_ct[1]))
  # Expected potential outcomes for all observations (Control group)
  expected_outcomes_ct <- sapply(1:n_obs, function(i) {
    mean(0.3*(X7_samples[i, ] - logit_X7_ct[i]))
  })
  
  mean(0.3*(X7_samples[1, ] - logit_X7_tx[1]))
  
  mean(0.3*(X7_samples[1, ] - logit_X7_tx[1])) - mean(0.3*(X7_samples[1, ] - logit_X7_ct[1]))
  # Expected potential outcomes for all observations (Treatment group)
  expected_outcomes_tx <- sapply(1:n_obs, function(i) {
    mean(0.3*(X7_samples[i, ] - logit_X7_tx[i]))
  })
  ...
  # 
  # # convert to tensorflow objects
  dat_temp <- as.matrix(simulated_full_data[, 1:5]) # without ITE_true
  # dat_temp[,4] <- dat_temp[,4] + 1  # binary treatment ordinal coded
  dat.tf = tf$constant(dat_temp, dtype = 'float32')
  ...
  # X1, X2, X3, X4 (Tr), X7 (Y)
  A =  matrix(c(
    0,   0,  0,  0,  1,    ## X1 impacts X7 (Y) directly
    0,   0,  0,  0,  1,    ## X2 impacts X7 (Y) directly
    0,   0,  0,  0 , 1,    ## X3 impacts X7 (Y) directly
    0,   0,  0,  0,  1,    ## X4 (Tr) X7 (Y) directly
    0,   0,  0,  0,  0),    ## X7 (Y) terminal node
...
train <- dgp_simulation_no_mediators(n_obs = n_obs, SEED = 123, rho = 0.1, doX = c(NA, NA, NA, NA, NA),
                        main_effect = main_effect, interaction_effect = interaction_effect)
global_min = train$min
global_max = train$max
data_type = train$type
...
beta_t <- 1.5
beta_X <- c(-0.5, 0.5, 0.2)  # direct effects of X1, X2, X3


data_X <- c(0.1, 0.1 , -0.1)

logit_X7_ct <- beta_t * 0 + data_X %*% beta_X

logit_X7_tx <- beta_t * 1  + data_X %*% beta_X

X7_ct_median <- h_y_inverse(0 - logit_X7_ct)

X7_tx_median <- h_y_inverse(0 - logit_X7_tx)

ITE_median <- X7_tx_median - X7_ct_median
ITE_median
...
X7_ct_median_linearh <- 1/3 * (0 - logit_X7_ct)

X7_tx_median_linearh <- 1/3 * (0 - logit_X7_tx)

ITE_median_linearh <- X7_tx_median_linearh - X7_ct_median_linearh
ITE_median_linearh



# other patient:
data_X <- c(0.4, 0.6 , -0.1)
...
par(mfrow=c(1,1))
hist(train$simulated_full_data$ITE_median, xlab = "ITE", breaks = 50)
