...
library(keras)
library(tidyverse)
source('utils_tf_ordinal.R')    # new tf file for ordinal data (predictors)

#### For TFP
library(tfprobability)
source('utils_tfp_ordinal.R')

##### Flavor of experiment ######

#### Saving the current version of the script into runtime
DIR = 'runs/experiment_5_all_ordinal_encoded_real_data_linear/run'
if (!dir.exists(DIR)) {
  dir.create(DIR, recursive = TRUE)
}
# Copy this file to the directory DIR
file.copy('/experiment_5_all_ordinal_encoded_real_data_linear.R', file.path(DIR, 'experiment_5_all_ordinal_encoded_real_data_linear.R'), overwrite=TRUE)
...
dgp <- function(n_obs, doX=c(NA, NA, NA), seed=123, data = NULL) {
  ...
  # define threshold encoded variables
  
  
  if (is.na(doX[1])){
    X_1_t1 = as.numeric(data$ENSO_ordinal %in% c(2,3))  # First threshold (2 or higher)
    X_1_t2 = as.numeric(data$ENSO_ordinal == 3)  # Second threshold (3 or higher)
  } else{
    X_1_t1 = rep(as.numeric(doX[1] %in% c(2,3)), n_obs)
    X_1_t2 = rep(as.numeric(doX[1] == 3), n_obs)
  }
  
  if (is.na(doX[2])){
    X_2_t1 = as.numeric(data$IOD_ordinal %in% c(2,3))  
    X_2_t2 = as.numeric(data$IOD_ordinal == 3)  
  } else{
    X_2_t1 = rep(as.numeric(doX[2] %in% c(2,3)), n_obs)
...
plot(1:length(train_loss), train_loss, type='l', main='Normal Training (green is valid)', ylim = c(2.4, 2.8))
...
fit_21_polr <- polr(x2 ~ x1, data = train$df_R, Hess=TRUE)
summary(fit_21_polr)



fit_21_Polr <- Polr(x2 ~ x1, data = train$df_R)
summary(fit_21_Polr)
...
sum(coef(fit_21_Polr))
sum(wes[c(1,3)])



train$df_R$x1 <- factor(train$df_R$x1, ordered = TRUE)
train$df_R$x2 <- factor(train$df_R$x2, ordered = TRUE)
train$df_R$x3 <- factor(train$df_R$x3, ordered = TRUE)
...
Polr(x2 ~ x1, data = train$df_R)


comp <- round(rbind(-c(coef(fit_21)[1], coef(fit_321)[2], 
                       coef(fit_21)[2]-coef(fit_21)[1], 
                       coef(fit_321)[3]-  coef(fit_321)[2], 
                       coef(fit_321)[4], 
                       coef(fit_321)[5]- coef(fit_321)[4]),
      wes), 3)
...
p <- ggplot(ws, aes(x=1:nrow(ws))) + 
  ...
  geom_hline(aes(yintercept=-(coef(fit_21)[2]-coef(fit_21)[1]), color='x1.2 --> x2'), linetype=2) +
  geom_hline(aes(yintercept=-(coef(fit_321)[2]), color='x1.1 --> x3'), linetype=2) +
  geom_hline(aes(yintercept=-(coef(fit_321)[3]- coef(fit_321)[2]), color='x1.2 --> x3'), linetype=2) +
  geom_hline(aes(yintercept=-(coef(fit_321)[4]), color='x2.1 --> x3'), linetype=2) +
  geom_hline(aes(yintercept=-(coef(fit_321)[5]- coef(fit_321)[4]), color='x2.2 --> x3'), linetype=2) +
...
if (TRUE){
  file_path <- file.path("runs/experiment_5_all_ordinal_encoded_real_data_linear/run/", basename(file_name))
...
shift_23 = shift_13 = cs_12 = xs = seq(-1,1,length.out=41)
...
plot(xs, cs_12, main='LS-Term (black DGP, red Ours)', 
     sub = 'Effect of x1 on x2',
     xlab='x1', col='red')
# abline(0, 2)

delta_0 = shift_13[idx0] - 0
plot(xs, shift_13 - delta_0, main='LS-Term (black DGP, red Ours)', 
