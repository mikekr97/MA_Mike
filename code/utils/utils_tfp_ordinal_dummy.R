...
do_dag_struct = function(param_model, MA, MA_encoded, doX = c(NA, NA, NA), num_samples=1042, data = train){
  ...
  xl = list() 
  s = tf$ones(c(num_samples, N))                                                ###### to store the final ordinal samples
  ...
  for (i in 1:N){
    ...
    if (cuts == 2){
      ts_lower <- as.numeric(as.array(ts) == cuts)  # First threshold (2 or lower)
    ...
    replace_cols <- which(mask_vector == 1)  # Should return c(1,2) for i=1
  ...
  s[,1] <- s_e_matrix[,1] + s_e_matrix[,2] * 2 + 1
  s[,2] <- s_e_matrix[,3] + s_e_matrix[,4] * 2 + 1
  s[,3] <- s_e_matrix[,5] + 1
