...
file.copy('/experiment_5_all_ordinal_dummy_encoded_real_data_linear.R', file.path(DIR, 'experiment_5_all_ordinal_dummy_encoded_real_data_linear.R'), overwrite=TRUE)
...
if (TRUE){
  file_path <- file.path("runs/experiment_5_all_ordinal_dummy_encoded_real_data_linear/run/", basename(file_name))
...
plot(1:length(xs), shift_12, main='LS-Term (black DGP, red Ours)', 
...
# estimated 20 thetas for the trafo
to_theta3(param_model(X)[,,3:dim(h_params)[3], drop = FALSE])

# estimated cut offs for x1 & x2 (each 2 cut offs because 3 levels)
to_theta3(param_model(X)[,,3:dim(h_params)[3], drop = FALSE])[1,1:2,1:2] 

# correspond to the intercepts from polr (here for x2)
fit_21
