...
create_param_model = function(MA, MA_encoded, hidden_features_I = c(2,2), len_theta=30, hidden_features_CS = c(2,2), train = NULL){
  ...
  input_layer <- layer_input(shape = list(ncol(MA)))                            # input_layer should not be used anymore. as of now, only SI-LS implemented
  ...
  if ('ci' %in% MA == TRUE) { # At least one 'ci' in model
    layer_sizes_I <- c(nrow(MA_encoded), hidden_features_I, ncol(MA_encoded))
    masks_I = create_masks(adjacency =  t(MA_encoded == 'ci'), hidden_features_I)
  ...
  if ('cs' %in% MA == TRUE) { # At least one 'cs' in model
    layer_sizes_CS <- c(nrow(MA_encoded), hidden_features_CS, ncol(MA_encoded))
    masks_CS = create_masks(adjacency =  t(MA_encoded == 'cs'), hidden_features_CS)
    h_CS = create_param_net(len_param = 1, input_layer=input_layer_encoded, layer_sizes = layer_sizes_CS, masks_CS, last_layer_bias=FALSE)
...
struct_dag_loss = function (t_i, h_params){
  ...
  # Reconstruct original ordinal data from encoded data (TensorFlow operations)
  x1_reconstructed_tf <- tf$squeeze(tf$linalg$matmul(t_i_encoded[, 1:2], tf$expand_dims(tf$constant(c(1, 2), dtype = tf$float32), axis = 1L))) + 1
  x2_reconstructed_tf <- tf$squeeze(tf$linalg$matmul(t_i_encoded[, 3:4], tf$expand_dims(tf$constant(c(1, 2), dtype = tf$float32), axis = 1L))) + 1
  x3_reconstructed_tf <- t_i_encoded[, 5] + 1
  
  # Stack TensorFlow tensors along columns
  t_i <- tf$stack(list(
    x1_reconstructed_tf,
    x2_reconstructed_tf,
    x3_reconstructed_tf
