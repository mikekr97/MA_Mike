...
train_indizes <- sample(1:nrow(raw_df), 0.8*nrow(raw_df))
...
# generate data
train <- dgp(nrow(train), seed=ifelse(SEED > 0, SEED, -1), data = train_df)
test <- dgp(nrow(test), seed=ifelse(SEED > 0, SEED, -1), data = test_df)
...
if (TRUE){
  ...
  d = dgp(nrow(train_df), doX=doX, data=train_df)$df_R
