...
DIR = 'runs/experiment_4_simulation_ComplexDGP_ComplexModel/run'
if (!dir.exists(DIR)) {
  dir.create(DIR, recursive = TRUE)
}
# Copy this file to the directory DIR
file.copy('MA_Mike/experiment_4_simulation_ComplexDGP_ComplexModel.R', file.path(DIR, 'experiment_4_simulation_ComplexDGP_ComplexModel.R'), overwrite=TRUE)
...
SEED = 123 #-1 #If seed > 0 then the seed is set
...
# generate data
train <- dgp(10000, seed=ifelse(SEED > 0, SEED, -1))
#test <- dgp(10000, seed=ifelse(SEED > 0, SEED, -1))
test <- dgp(10000, seed=42)
...
if (FALSE){
  ...
  file_path <- file.path("runs/experiment_4_simulation_ComplexDGP_ComplexModel/run/", basename(file_name))
...
kde_x1 <- density(train$df_R$x1, bw = "SJ")  
kde_x2 <- density(train$df_R$x2, bw = "SJ")

par(mfrow=c(1,2))
hist(train$df_R$x1, main = "X1", xlab = "NP", prob=TRUE)
lines(kde_x1, col="red")
hist(train$df_R$x2, main = "X2", xlab = "Ural", prob=TRUE)
...
fit_3 <- lm(x3 ~ x1 + x2, data=train$df_R)
fit_4 <- lm(x4 ~ x1 + x2 + x3, data=train$df_R)
...
# get the likelihoods for the conditional distributions X3 and X4
...
nll_normal_density_3 <- function(data) {
  mu <- fit_3$coefficients[1] + fit_3$coefficients[2]*data$df_R$x1 + fit_3$coefficients[3]*data$df_R$x2
  sigma <- summary(fit_3)$sigma
  log_dens <- log(dnorm(data$df_R$x3, mean=mu, sd=sigma))
  nll <- -mean(log_dens)
  return (nll)
}

nll_normal_density_3(train)

# X4 ~ N(a + b1*X1 + b2*X2 + b3*X3, sigma^2)

nll_normal_density_4 <- function(data) {
  mu <- fit_4$coefficients[1] + fit_4$coefficients[2]*data$df_R$x1 + 
    fit_4$coefficients[3]*data$df_R$x2 + fit_4$coefficients[4]*data$df_R$x3
  sigma <- summary(fit_3)$sigma
  log_dens <- log(dnorm(data$df_R$x4, mean=mu, sd=sigma))
  nll <- -mean(log_dens)
  return (nll)
}

nll_normal_density_4(train)



##### NLL for TRAM-DAG X3 and X4



struct_dag_loss_x3_x4 = function (t_i, h_params){
  ...
  if (length(cont_dims) != 0){
    ...
    NLL_x1 = 0 - tf$reduce_mean(log_latent_density[,1] + log_hdash[,1])
    NLL_x2 = 0 - tf$reduce_mean(log_latent_density[,2] + log_hdash[,2])
    NLL_x3 = 0 - tf$reduce_mean(log_latent_density[,3] + log_hdash[,3])
    NLL_x4 = 0 - tf$reduce_mean(log_latent_density[,4] + log_hdash[,4])
    
    return(list(nll_x1 = NLL_x1, nll_x2= NLL_x2, nll_x3 = NLL_x3, nll_x4= NLL_x4))
  }
...
# Train data
h_params = param_model(train$df_orig)
tram_train_nll <- struct_dag_loss_x3_x4(train$df_orig, h_params)
# sum( as.numeric(tram_train_nll$nll_x1), as.numeric(tram_train_nll$nll_x2),
#       as.numeric(tram_train_nll$nll_x3), as.numeric(tram_train_nll$nll_x4))/4
# struct_dag_loss(train$df_orig, h_params)

nll_comparison_train <- data.frame(
  NLL_X3 = c(nll_normal_density_3(train), as.numeric(tram_train_nll$nll_x3)),
  NLL_X4 = c(nll_normal_density_4(train), as.numeric(tram_train_nll$nll_x4))
)

rownames(nll_comparison_train) <- c('Linear Model', 'TRAM-DAG')
round(nll_comparison_train, 2)


# Test data
h_params = param_model(test$df_orig)
tram_test_nll <- struct_dag_loss_x3_x4(test$df_orig, h_params)

nll_comparison_test <- data.frame(
  NLL_X3 = c(nll_normal_density_3(test), as.numeric(tram_test_nll$nll_x3)),
  NLL_X4 = c(nll_normal_density_4(test), as.numeric(tram_test_nll$nll_x4))
)

rownames(nll_comparison_test) <- c('Linear Model', 'TRAM-DAG')
round(nll_comparison_test, 2)
# 
# 
# 
# #==============================================================================
...
shift_23 = shift_13 = shift_14  = shift_24 = cs_24 = xs = seq(-1,1,length.out=41)
idx0 = which(xs == 0) #Index of 0 xs needs to be odd
...
plot(xs, cs_24, type='l', col='red', lwd=2, xlab='x2', ylab='f(x2)', main='b24(x1) complex')
